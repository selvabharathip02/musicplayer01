<script>
  ...
  const navItems = ['Home', 'Library', 'Playlist', 'About Music', 'Contact'];
  ...
  function playPreview() {
    player.playTrack(featuredTrack, [featuredTrack]);
  }

  function togglePreview() {
    if (playerState.current?.id === featuredTrack.id && playerState.playing) {
      player.pause();
    } else {
      playPreview();
    }
...
<svelte:head>
  <title>Pulse Music - Premium Landing Template</title>
  <meta name="description" content="A modern landing template for Pulse Music with a dark theme, glassmorphic design, and vibrant orange highlights." />
</svelte:head>
...
<div class="min-h-screen bg-[#10131d] text-slate-100 font-sans selection:bg-orange-500/30 selection:text-white">
  <div class="relative overflow-hidden bg-[radial-gradient(circle_at_12%_8%,rgba(255,107,0,0.22),transparent_35%),radial-gradient(circle_at_92%_18%,rgba(255,140,0,0.2),transparent_35%),linear-gradient(145deg,#10131d_0%,#181b2b_58%,#0e1119_100%)] min-h-screen">
    ...
    <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 relative z-10">
      ...
      <header class="mb-8">
        <div class="flex items-center justify-between rounded-full border border-white/10 bg-[#151927]/85 px-4 sm:px-6 py-3.5 shadow-xl backdrop-blur-xl">
          <a href={`${base}/template`} class="flex items-center gap-3 group">
            ...
            <div>
              ...
              <span class="text-[9px] sm:text-[10px] uppercase tracking-widest text-orange-400 font-bold block">Worship Template</span>
            </div>
          </a>

          <!-- Desktop Navigation Menu -->
          <nav class="hidden md:flex items-center gap-2 text-sm font-medium">
            <a href={`${base}/template`} class="px-4 py-2 rounded-full bg-orange-500 text-slate-950 font-bold shadow-lg shadow-orange-500/25 transition cursor-pointer">Home</a>
        ...
        {#if mobileMenuOpen}
          <nav class="md:hidden mt-3 rounded-3xl border border-white/10 bg-[#151927]/95 p-4 shadow-2xl backdrop-blur-xl flex flex-col gap-2">
            <a href={`${base}/template`} onclick={() => mobileMenuOpen = false} class="px-4 py-2.5 rounded-2xl bg-orange-500 text-slate-950 font-bold text-sm">Home</a>
      ...
      <!-- Hero Section -->
      <section class="mb-12 rounded-[2.5rem] border border-white/10 bg-white/6 p-6 sm:p-12 shadow-[18px_18px_45px_rgba(4,6,15,0.42),-12px_-12px_36px_rgba(255,255,255,0.025)] backdrop-blur-xl">
        <div class="grid items-center gap-10 lg:grid-cols-12">
          <div class="lg:col-span-7">
            ...
            <div class="inline-flex items-center gap-2 rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.15em] text-orange-400 backdrop-blur-md mb-6">
              ...
              Feel the Music Anytime 🎶
            </div>

            <!-- Bold Headline -->
            <h1 class="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl leading-[1.12]">
              Listen & Enjoy Your <br class="hidden sm:inline" />
              <span class="bg-gradient-to-r from-orange-400 via-orange-500 to-amber-400 bg-clip-text text-transparent">Favorite</span> Music 🎧 
            </h1>

            <!-- Short Description -->
            <p class="mt-5 text-sm sm:text-base leading-7 text-slate-300 max-w-2xl">
              Stream your favorite songs with high quality audio, create custom playlists, and enjoy nonstop spiritual and worship music anytime, anywhere with Pulse Music.
            </p>

            <!-- CTA Buttons -->
            <div class="mt-8 flex flex-wrap items-center gap-4">
              <a href={`${base}/loginpage`} class="flex items-center gap-2 rounded-full bg-orange-500 px-7 py-3.5 text-sm font-bold text-slate-950 shadow-xl shadow-orange-500/25 transition-all hover:bg-orange-400 hover:scale-105 active:scale-95">
                Get Started ▶
              </a>
            </div>
            ...
            <div class="mt-10 grid grid-cols-3 gap-4 border-t border-white/10 pt-8 max-w-lg">
              <div class="rounded-3xl bg-white/4 p-4 text-center shadow-[inset_4px_4px_12px_rgba(0,0,0,0.2),inset_-4px_-4px_12px_rgba(255,255,255,0.025)]">
                <p class="text-2xl sm:text-3xl font-extrabold text-white">Unlimited</p>
                <p class="mt-1 text-[11px] uppercase tracking-[0.15em] text-slate-400 font-semibold">Songs</p>
              </div>
              <div class="rounded-3xl bg-white/4 p-4 text-center shadow-[inset_4px_4px_12px_rgba(0,0,0,0.2),inset_-4px_-4px_12px_rgba(255,255,255,0.025)]">
                <p class="text-2xl sm:text-3xl font-extrabold text-orange-400">10k+</p>
                <p class="mt-1 text-[11px] uppercase tracking-[0.15em] text-slate-400 font-semibold">Tracks</p>
              </div>
              <div class="rounded-3xl bg-white/4 p-4 text-center shadow-[inset_4px_4px_12px_rgba(0,0,0,0.2),inset_-4px_-4px_12px_rgba(255,255,255,0.025)]">
                <p class="text-2xl sm:text-3xl font-extrabold text-white">24/7</p>
                <p class="mt-1 text-[11px] uppercase tracking-[0.15em] text-slate-400 font-semibold">Music</p>
              </div>
            </div>
          </div>
              ...
              <div class="relative overflow-hidden rounded-4xl border border-white/10 bg-[#151927]/80 p-3.5 shadow-2xl shadow-black/40">
                <div class="relative overflow-hidden rounded-3xl bg-slate-950">
                  <img 
                    ...
                    alt="Hero Preview" 
                    class="h-[380px] w-full object-cover object-center opacity-90 transition-transform duration-700 hover:scale-105"
                  />
                ...
                <div class="mt-3 flex items-center justify-between rounded-3xl bg-white/4 p-4 shadow-[inset_4px_4px_12px_rgba(0,0,0,0.2),inset_-4px_-4px_12px_rgba(255,255,255,0.025)]">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-slate-400">320kbps lossless streaming</p>
                    </div>
                  </div>
                  <a href={`${base}/menu`} class="rounded-full bg-orange-500 px-4 py-1.5 text-xs font-bold text-slate-950 transition hover:bg-orange-400 hover:scale-105">Listen</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="services" class="space-y-8 pb-12">
        <div class="rounded-[2.5rem] border border-white/10 bg-white/6 p-8 shadow-[18px_18px_45px_rgba(4,6,15,0.38),-10px_-10px_30px_rgba(255,255,255,0.025)] text-center max-w-2xl mx-auto">
          <p class="text-xs uppercase tracking-[0.2em] text-orange-400 font-bold">Features</p>
          <h2 class="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl">
            Music Player Features 🎶
          </h2>
          <p class="mt-2 text-sm text-slate-300 leading-6">
            Enjoy powerful music features engineered for the ultimate spiritual listening experience.
          </p>
        </div>

        <div class="grid gap-6 md:grid-cols-3">
          <!-- Feature Card 1 -->
          <div class="group rounded-[2.5rem] border border-white/10 bg-slate-950/80 p-8 shadow-xl transition-all duration-300 hover:border-orange-500/40 hover:bg-slate-900/90 hover:-translate-y-1.5">
            <div class="mb-6 flex h-14 w-14 items-center justify-center rounded-3xl bg-white/4 text-orange-400 shadow-[inset_4px_4px_12px_rgba(0,0,0,0.2),inset_-4px_-4px_12px_rgba(255,255,255,0.025)] transition-colors group-hover:bg-orange-500 group-hover:text-slate-950">
              <svg class="h-7 w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
              </svg>
            </div>
            <h3 class="text-lg font-bold text-white group-hover:text-orange-400 transition-colors">High Quality Audio 🔊</h3>
            <p class="mt-3 text-sm leading-6 text-slate-400">
              Experience crystal clear sound with immersive high-quality audio streaming tailored for worship tracks.
            </p>
          </div>
          ...
          <div class="group rounded-[2.5rem] border border-white/10 bg-slate-950/80 p-8 shadow-xl transition-all duration-300 hover:border-orange-500/40 hover:bg-slate-900/90 hover:-translate-y-1.5">
            ...
            <h3 class="text-lg font-bold text-white group-hover:text-orange-400 transition-colors">Offline & Download 📥</h3>
            <p class="mt-3 text-sm leading-6 text-slate-400">
              Download your favorite tracks and enjoy nonstop listening anywhere without internet interruptions.
            </p>
          </div>
          ...
          <div class="group rounded-[2.5rem] border border-white/10 bg-slate-950/80 p-8 shadow-xl transition-all duration-300 hover:border-orange-500/40 hover:bg-slate-900/90 hover:-translate-y-1.5">
            ...
            <h3 class="text-lg font-bold text-white group-hover:text-orange-400 transition-colors">Smart Recommendations 🤖</h3>
            <p class="mt-3 text-sm leading-6 text-slate-400">
              Get personalized recommendations tailored to your taste and worship listening history.
            </p>
          </div>
        </div>
      </section>
      ...
      <!-- Footer -->
      <footer id="contact" class="py-8 border-t border-white/10 text-center text-xs text-slate-400">
